...
select progression in $PROGRESSIONS; do
    if [ "$progression" = "linear" ]; then
        keyframes=`seq 0 $(echo "scale=5; 45 / $frames" | bc -l) 45`
        break
...
{ for i in $keyframes; do
    $DIR/target/release/tree -c --leaf-color '#090' -b 6 -i 6 -a 25 -l 230 -p 960,1080 -s $i | $DIR/target/release/turtle-svg -w 1920 -h 1080 | convert svg: png:- 
  done
